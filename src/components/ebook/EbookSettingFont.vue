    ...
    <div class="setting-wrapper" v-show="getMenuVisible && getSettingVisible === 0">
      ...
      <div class="setting-font-family" >
        <div class="setting-font-family-text-wrapper">
          <span class="setting-font-family-text"  @click.stop="showFontFamilySetting" >{{getDefaultFontFamily}}</span>
        </div>
...
export default {
  ...
  mounted () {
    // console.log('getDefaultFontSize', this.getDefaultFontSize)
  },
  watch: {
    getSettingVisible (v) {
      if (v === 0) {
  ...
  methods: {
    ...
    setFontSize (fontSize) {
      this.setDefaultFontSize(fontSize).then(() => {
        // this.switchTheme()
        saveFontSize(this.getFileName, fontSize)
        this.getCurrentBook.rendition.themes.fontSize(this.getDefaultFontSize)
